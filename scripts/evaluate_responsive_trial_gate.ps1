...
function Get-ResponsiveTrialGateMarkdown {
    ...
    $lines = @(
        ...
        "- Synthetic-only evidence excluded from promotion: $($Gate.synthetic_only_evidence_excluded_from_promotion)",
        "",
        "## Registry Context",
        "",
        "- Total registered sessions: $($Gate.total_registered_sessions)",
        "- Real sessions considered live evidence: $($Gate.real_registered_sessions)",
        "- Synthetic sessions present: $($Gate.synthetic_registered_sessions)",
        ...
        "",
        "- Total grounded conservative sessions: $($Gate.conservative_evidence_counts.total_grounded_count)",
        "- Real grounded conservative sessions: $($Gate.conservative_evidence_counts.real_grounded_count)",
        "- Grounded conservative too-quiet cases: $($Gate.conservative_evidence_counts.total_grounded_too_quiet_count)",
        "- Real grounded conservative too-quiet cases: $($Gate.conservative_evidence_counts.real_grounded_too_quiet_count)",
        "- Real grounded conservative appropriately-conservative cases: $($Gate.conservative_evidence_counts.real_grounded_appropriately_conservative_count)",
        "- Real distinct grounded conservative too-quiet pair IDs: $($Gate.conservative_evidence_counts.real_distinct_grounded_too_quiet_pair_ids_count)",
        "- Real grounded conservative shadow responsive-candidate cases: $($Gate.conservative_evidence_counts.real_shadow_responsive_candidate_count)",
        "",
        "## Responsive Risk Evidence",
        "",
        "- Total grounded responsive sessions: $($Gate.responsive_risk_evidence_counts.total_grounded_count)",
        "- Real grounded responsive sessions: $($Gate.responsive_risk_evidence_counts.real_grounded_count)",
        "- Grounded responsive too-reactive cases: $($Gate.responsive_risk_evidence_counts.total_grounded_too_reactive_count)",
        "- Real grounded responsive too-reactive cases: $($Gate.responsive_risk_evidence_counts.real_grounded_too_reactive_count)",
        "- Real grounded responsive appropriately-conservative cases: $($Gate.responsive_risk_evidence_counts.real_grounded_appropriately_conservative_count)",
        "",
        "## Thresholds",
        ""
    )
...
foreach ($entry in $rawEntries) {
    ...
    $normalizedEntries += [pscustomobject]@{
        ...
        synthetic_fixture = Test-SyntheticRegistryEntry -Entry $entry
        fixture_id = [string](Get-ObjectPropertyValue -Object $entry -Name "fixture_id" -Default "")
    }
}

$realEntries = @($normalizedEntries | Where-Object { -not $_.synthetic_fixture })
$syntheticEntries = @($normalizedEntries | Where-Object { $_.synthetic_fixture })
$decisionScopeEntries = @(
    if ($IncludeSyntheticEvidenceForValidation) { $normalizedEntries } else { $realEntries }
)
...
$decisionGroundedEntries = @($decisionScopeEntries | Where-Object { $_.session_is_tuning_usable })
$realGroundedEntries = @($realEntries | Where-Object { $_.session_is_tuning_usable })
$syntheticGroundedEntries = @($syntheticEntries | Where-Object { $_.session_is_tuning_usable })

$conservativeGroundedAll = @($normalizedEntries | Where-Object { $_.treatment_profile -eq "conservative" -and $_.session_is_tuning_usable })
$conservativeGroundedReal = @($realEntries | Where-Object { $_.treatment_profile -eq "conservative" -and $_.session_is_tuning_usable })
$conservativeGroundedDecision = @($decisionScopeEntries | Where-Object { $_.treatment_profile -eq "conservative" -and $_.session_is_tuning_usable })
...
$responsiveGroundedAll = @($normalizedEntries | Where-Object { $_.treatment_profile -eq "responsive" -and $_.session_is_tuning_usable })
$responsiveGroundedReal = @($realEntries | Where-Object { $_.treatment_profile -eq "responsive" -and $_.session_is_tuning_usable })
$responsiveGroundedDecision = @($decisionScopeEntries | Where-Object { $_.treatment_profile -eq "responsive" -and $_.session_is_tuning_usable })
...
if ($responsiveTooReactiveDecision.Count -gt 0 -and $responsiveAppropriateDecision.Count -gt 0) {
    ...
    $explanation = "Grounded responsive evidence is mixed: at least one responsive session looks too reactive and at least one looks acceptably bounded, so an operator needs to review the artifacts before another live choice."
...
elseif ($responsiveTooReactiveDecision.Count -ge $minGroundedResponsiveTooReactiveSessionsForRevert) {
    ...
    $explanation = "Grounded responsive evidence already shows overreaction, so the live treatment profile should revert to conservative instead of scheduling another responsive trial."
    $supportingEntries = $responsiveTooReactiveDecision
}
elseif ($conservativeTooReactiveDecision.Count -gt 0) {
    $gateVerdict = "manual-review-needed"
    $nextLiveAction = "manual-review-needed"
    $explanation = "Grounded conservative evidence includes at least one too-reactive assessment, which should not be promoted or ignored without manual review."
...
elseif ($conservativeTooQuietDecision.Count -gt 0 -and $conservativeAppropriateDecision.Count -gt 0) {
    $gateVerdict = "manual-review-needed"
    $nextLiveAction = "manual-review-needed"
    $explanation = "Grounded conservative evidence is mixed between too-quiet and appropriately-conservative outcomes, so the first responsive trial should stay blocked until the artifacts are reviewed."
...
elseif ($candidateThresholdsMet -and ($candidateShadowTooReactiveDecision.Count -gt 0 -or $candidateShadowManualDecision.Count -gt 0)) {
    $gateVerdict = "manual-review-needed"
    $nextLiveAction = "manual-review-needed"
    $explanation = "The conservative too-quiet threshold was met, but shadow review on those grounded sessions still raised overreaction or manual-review concerns, so the first live responsive trial should not be scheduled blindly."
...
elseif ($candidateThresholdsMet -and $candidateShadowReviewsDecision.Count -gt 0 -and $candidateShadowResponsiveDecision.Count -eq 0) {
    ...
    $explanation = "Grounded conservative sessions are trending too quiet, but the available shadow reviews did not yet support responsive as the next candidate, so more conservative evidence should be collected first."
    $supportingEntries = $conservativeTooQuietDecision
}
elseif ($candidateThresholdsMet) {
    $gateVerdict = "open"
    $nextLiveAction = "responsive-trial-allowed"
    $explanation = "Repeated grounded conservative sessions stayed too quiet across distinct pair runs without grounded evidence that conservative is already behaving acceptably, so one bounded live responsive trial is justified."
    $supportingEntries = $conservativeTooQuietDecision
}
elseif (-not $IncludeSyntheticEvidenceForValidation -and $realGroundedEntries.Count -eq 0 -and $syntheticGroundedEntries.Count -gt 0) {
    $gateVerdict = "closed"
    $nextLiveAction = "responsive-trial-not-allowed"
    $explanation = "Synthetic fixtures can validate the workflow, but they cannot unlock the first live responsive trial. Real grounded conservative evidence is still missing."
    $supportingEntries = $syntheticGroundedEntries
}
elseif ($decisionGroundedEntries.Count -eq 0) {
    $gateVerdict = "closed"
    $nextLiveAction = "responsive-trial-not-allowed"
    if ($decisionScopeEntries.Count -eq 0) {
        $explanation = "No pair sessions have been registered yet, so there is no evidence to justify the first live responsive trial."
    }
    else {
        $explanation = "The registry is still dominated by insufficient-data or weak-signal evidence, so the first live responsive trial remains blocked."
    }
    $supportingEntries = if ($decisionScopeEntries.Count -gt 0) { $decisionScopeEntries } else { @() }
}
elseif (
    ...
    $explanation = "Grounded conservative sessions already show bounded, acceptable live behavior without grounded too-quiet evidence, so conservative should remain the default live treatment profile."
    $supportingEntries = $conservativeAppropriateDecision
}
elseif ($conservativeGroundedDecision.Count -ge 1) {
    $gateVerdict = "closed"
    $nextLiveAction = "collect-more-conservative-evidence"
    $explanation = "There is some grounded conservative evidence, but not enough repeated real evidence yet to justify opening the first live responsive trial."
    $supportingEntries = $conservativeGroundedDecision
}
else {
    $gateVerdict = "manual-review-needed"
    $nextLiveAction = "manual-review-needed"
    $explanation = "The available evidence does not fit a clean responsive-trial promotion rule, so the registry and pair artifacts need manual review."
...
switch ($nextLiveAction) {
    "responsive-trial-not-allowed" {
        if (-not $IncludeSyntheticEvidenceForValidation -and $realEntries.Count -eq 0) {
            $missingEvidence += "At least one real live conservative pair session must be registered before responsive can be considered."
        }
        if ($realGroundedEntries.Count -eq 0) {
            $missingEvidence += "At least one real conservative pair needs tuning-usable or strong-signal evidence with meaningful human presence."
        }
        if ($realGroundedEntries.Count -lt $minGroundedConservativeSessionsForResponsiveTrial) {
            $missingEvidence += "At least $minGroundedConservativeSessionsForResponsiveTrial real grounded conservative sessions are required before the first live responsive trial."
        }
        if ($conservativeTooQuietReal.Count -lt $minGroundedConservativeTooQuietSessionsForResponsiveTrial) {
            $missingEvidence += "At least $minGroundedConservativeTooQuietSessionsForResponsiveTrial real grounded conservative too-quiet sessions are required before responsive can open."
        }
        if ($IncludeSyntheticEvidenceForValidation -eq $false -and $syntheticGroundedEntries.Count -gt 0) {
            $missingEvidence += "Synthetic fixture evidence is validation-only and must not be used as live promotion evidence."
        }
    }
    "collect-more-conservative-evidence" {
        if ($conservativeGroundedReal.Count -lt $minGroundedConservativeSessionsForResponsiveTrial) {
            $missingEvidence += "Collect another real grounded conservative pair so the promotion rule spans more than one live session."
        }
        if ($conservativeTooQuietReal.Count -lt $minGroundedConservativeTooQuietSessionsForResponsiveTrial) {
            $missingEvidence += "Collect another real grounded conservative too-quiet session before opening responsive."
        }
...
$promotionEvidenceScope = if ($IncludeSyntheticEvidenceForValidation) { "validation-includes-synthetic" } else { "real-only" }
...
$gate = [ordered]@{
    ...
    synthetic_only_evidence_excluded_from_promotion = -not $IncludeSyntheticEvidenceForValidation
    total_registered_sessions = $normalizedEntries.Count
    real_registered_sessions = $realEntries.Count
    synthetic_registered_sessions = $syntheticEntries.Count
