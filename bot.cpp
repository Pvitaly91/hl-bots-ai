...
static void BotSpawnInit_TimersAndPhysics( bot_t &pBot )
{
   ...
   pBot.movement_mode = BOT_MOVE_UNSET;
   pBot.trace_last_movement_mode = BOT_MOVE_UNSET;
   pBot.f_movement_trace_time = 0.0f;
   ...
   pBot.trace_last_ammo_need = BOT_AMMO_NEED_NONE;
...
static void BotSpawnInit_CombatState( bot_t &pBot )
{
...
static void BotSpawnInit_SpecialAbilities( bot_t &pBot )
{
...
static void BotSpawnInit_BehaviorAndInventory( bot_t &pBot )
{
...
{
   BotSpawnInit_TimersAndPhysics(pBot);
   BotSpawnInit_CombatState(pBot);
   BotSpawnInit_SpecialAbilities(pBot);
   BotSpawnInit_BehaviorAndInventory(pBot);
}
...
//
static int GetSpecificTeam_CountTeamMembers(int team_index, qboolean only_count_bots)
{
   int count = 0;
   ...
   for(int j = 1; j <= gpGlobals->maxClients; j++)
   {
      ...
      // match team
      if(!stricmp(UTIL_GetTeam(pClient, teamname, sizeof(teamname)), g_team_names[team_index]))
      {
         count++;
      }
   }
...
//
static void GetSpecificTeam_CompareAndTrack(int count, int i, qboolean get_smallest, int &find_count, int &find_index)
{
   if(get_smallest)
   {
      // smaller?
...
char * GetSpecificTeam(char * teamstr, size_t slen, qboolean get_smallest, qboolean get_largest, qboolean only_count_bots)
{
   ...
   for(int i = 0; i < MAX_TEAMS; i++)
   {
      ...
      {
         int count = GetSpecificTeam_CountTeamMembers(i, only_count_bots);

         GetSpecificTeam_CompareAndTrack(count, i, get_smallest, find_count, find_index);
      }
   }
...
static void BotCreate_SelectSkin(const char *skin, char *c_skin, int sizeof_c_skin)
{
   char balanceskin[MAX_TEAMNAME_LENGTH];
   int max_skin_index = number_skins;
   ...
   if (skin == NULL || skin[0] == '\0')
   {
      int index = RANDOM_LONG2(0, number_skins-1);
      ...
      safe_strcopy(c_skin, sizeof_c_skin, bot_skins[index].model_name);
   }
   else
   {
      safe_strcopy(c_skin, sizeof_c_skin, skin);
   }

   for (int i = 0; c_skin[i] != 0; i++)
      c_skin[i] = tolower( c_skin[i] );  // convert to all lowercase
}


static qboolean BotCreate_ValidateModelFile(const char *c_skin)
{
...
static void BotCreate_SelectName(const char *name, const char *c_skin,
   char *c_name, int sizeof_c_name)
{
   if ((name != NULL) && (*name != 0))
   {
      safe_strcopy(c_name, sizeof_c_name, name);
   }
   else
   {
      if (number_names > 0)
         BotPickName( c_name, sizeof_c_name );
      else
      {
         // copy the name of the model to the bot's name...
         safe_strcopy(c_name, sizeof_c_name, c_skin);
      }
   }
}


static void BotCreate_ProcessSkillAndColors(int &skill, int &top_color,
   int &bottom_color, qboolean &got_skill_arg, char *c_topcolor,
   int sizeof_topcolor, char *c_bottomcolor, int sizeof_bottomcolor)
{
   if (skill >= 1 && skill <= 5)
   {
      got_skill_arg = TRUE;
   ...
   safevoid_snprintf(c_topcolor, sizeof_topcolor, "%d", top_color);
   safevoid_snprintf(c_bottomcolor, sizeof_bottomcolor, "%d", bottom_color);
}


static void BotCreate_SanitizeNameAndTags(char *c_name, int sizeof_c_name,
   int &skill, qboolean got_skill_arg)
{
   ...
   {
      char tmp[BOT_NAME_LEN];

      safevoid_snprintf(tmp, sizeof(tmp), "[lvl%d]%s", skill, c_name);
      tmp[sizeof(tmp)-1] = 0;  // make sure c_name is null terminated
      safe_strcopy(c_name, sizeof_c_name, tmp);
      c_name[sizeof_c_name-1] = 0;  // make sure c_name is null terminated
   }
}


static int BotCreate_CreateFakeClient(char *c_name, char *c_skin,
   char *c_topcolor, char *c_bottomcolor, int top_color,
   int bottom_color, edict_t *&BotEnt)
{
...
static void BotCreate_InitializeState(int index, edict_t *BotEnt,
   const char *c_skin, int top_color, int bottom_color, int cfg_bot_index,
   int skill)
{
...
void BotCreate( const char *skin, const char *name, int skill, int top_color, int bottom_color, int cfg_bot_index )
{
   ...
   BotCreate_SelectSkin(skin, c_skin, sizeof(c_skin));

   if (!BotCreate_ValidateModelFile(c_skin))
      return;

   BotCreate_SelectName(name, c_skin, c_name, sizeof(c_name));

   BotCreate_ProcessSkillAndColors(skill, top_color, bottom_color, got_skill_arg,
      c_topcolor, sizeof(c_topcolor), c_bottomcolor, sizeof(c_bottomcolor));

   BotCreate_SanitizeNameAndTags(c_name, sizeof(c_name), skill, got_skill_arg);

   edict_t *BotEnt;
   int index = BotCreate_CreateFakeClient(c_name, c_skin, c_topcolor, c_bottomcolor,
      top_color, bottom_color, BotEnt);
   if (index < 0)
      return;

   BotCreate_InitializeState(index, BotEnt, c_skin, top_color, bottom_color, cfg_bot_index, skill);
}
...
static qboolean BotFindItemIsAmmoPickup(const edict_t *pItem)
{
   return pItem && pItem->v.classname != 0 &&
      GetAmmoItemFlag(STRING(pItem->v.classname)) != 0;
}


static bot_ammo_need_t BotFindItemGetAmmoSourceNeed(bot_t &pBot,
   const edict_t *pItem, bot_ammo_need_info_t *pInfo)
{
   if (!pItem || pItem->v.classname == 0)
      return BOT_AMMO_NEED_NONE;

   const char *item_name = STRING(pItem->v.classname);
   int ammo_item_flags = GetAmmoItemFlag(item_name);
   if (ammo_item_flags)
      return BotGetAmmoPickupNeed(pBot, ammo_item_flags, pInfo);

   int weapon_item_flags = GetWeaponItemFlag(item_name);
   if (weapon_item_flags)
      return BotGetWeaponRepickupAmmoNeed(pBot, weapon_item_flags, pInfo);

   return BOT_AMMO_NEED_NONE;
}


static qboolean BotFindItemIsCoordinatedPickup(const edict_t *pItem)
{
   return BotFindItemIsWeaponPickup(pItem) || BotFindItemIsAmmoPickup(pItem);
}
...
static qboolean BotFindItemPickupMatchesWaypoint(const edict_t *pItem,
   int waypoint_index)
{
   if (!pItem || waypoint_index < 0 || waypoint_index >= num_waypoints ||
       (pItem->v.origin - waypoints[waypoint_index].origin).Length() > 64.0f)
      return FALSE;

   if (BotFindItemIsAmmoPickup(pItem))
      return (waypoints[waypoint_index].flags & W_FL_AMMO) != 0;

   return BotFindItemIsWeaponPickup(pItem) &&
      (waypoints[waypoint_index].flags & W_FL_WEAPON) != 0;
}
...
static qboolean BotFindItemClaimedByVisibleBot(const bot_t &pBot,
   edict_t *pItem)
{
   if (!BotFindItemIsCoordinatedPickup(pItem))
      return FALSE;
   ...
   for (int i = 0; i < 32; i++)
   {
      ...
      qboolean claims_item = otherBot.pBotPickupItem == pItem;
       ...
       if (!claims_item &&
           (otherBot.wpt_goal_type == WPT_GOAL_WEAPON ||
            otherBot.wpt_goal_type == WPT_GOAL_AMMO))
      {
...
static qboolean BotFindItemCheckExpiry(bot_t &pBot)
{
   ...
   if (pBot.pBotPickupItem)
   {
      ...
      bot_ammo_need_t ammo_need = BotFindItemGetAmmoSourceNeed(pBot,
         pBot.pBotPickupItem, &ammo_info);
      if (ammo_need != BOT_AMMO_NEED_NONE &&
          MapProfileShouldYieldToStrategicMovement(pBot))
      {
         pBot.f_last_item_found = -1;
         pBot.pBotPickupItem = NULL;
      }
      else if (ammo_need == BOT_AMMO_NEED_NONE &&
               (BotFindItemIsAmmoPickup(pBot.pBotPickupItem) ||
                (pBot.pBotPickupItem->v.classname != 0 &&
                 GetWeaponItemFlag(STRING(
                    pBot.pBotPickupItem->v.classname)) != 0 &&
                 ammo_info.reason != BOT_AMMO_REASON_IRRELEVANT)))
      {
         // The pool can fill from another pickup while this target is active.
         pBot.f_last_item_found = -1;
         pBot.pBotPickupItem = NULL;
      }
   }
   ...
   // Weak/newly spawned bots keep rescanning so a newly available close
   // upgrade can preempt a farther physical target.
   if (pBot.pBotPickupItem && !pBot.b_only_has_weak_weapons &&
       !BotIsInSpawnWeaponSearchWindow(pBot) &&
       BotFindItemGetAmmoSourceNeed(pBot, pBot.pBotPickupItem, NULL) ==
          BOT_AMMO_NEED_NONE)
      return TRUE;
...
static qboolean BotFindItemCheckFuncLadder(bot_t &pBot, edict_t *pent,
                                            Vector &entity_origin, const Vector &vecStart)
{
...
// Returns: 0=no match, 1=matched not usable, 2=can_pickup
static int BotFindItemCheckCharger(bot_t &pBot, edict_t *pent, const char *item_name,
                                    float distance, int angle_to_entity, qboolean is_health)
{
   edict_t *pEdict = pBot.pEdict;
   const char *charger_name = is_health ? "func_healthcharger" : "func_recharge";
   float stat = is_health ? pEdict->v.health : pEdict->v.armorvalue;
   float stat_max = is_health ? (float)VALVE_MAX_NORMAL_HEALTH : (float)VALVE_MAX_NORMAL_BATTERY;
   qboolean &b_use_station = is_health ? pBot.b_use_health_station : pBot.b_use_HEV_station;
   float &f_use_time = is_health ? pBot.f_use_health_time : pBot.f_use_HEV_time;

   if (strcmp(charger_name, item_name) != 0 || !(stat < stat_max) || pent->v.frame != 0)
   {
   ...
   return 1;  // matched not usable
}


static qboolean BotFindItemCheckFuncButton(bot_t &pBot, edict_t *pent, const char *item_name,
                                            TraceResult &tr, float distance, int angle_to_entity)
{
...
static qboolean BotFindItemCheckWeapon(bot_t &pBot, edict_t *pent,
   int itemflag, bot_ammo_need_info_t *pAmmoInfo)
{
   ...
   if(found)
   {
      // If already carried, this entity is useful only as an ammo source
      // explicitly described by the weapon metadata.
      if(pEdict->v.weapons & (1u<<pSelect[select_index].iId))
      {
         return BotGetWeaponRepickupAmmoNeed(pBot, itemflag, pAmmoInfo) !=
            BOT_AMMO_NEED_NONE;
      }
      else
      {
...
static qboolean BotFindItemCheckAmmo(bot_t &pBot, edict_t *pent, int itemflag,
   bot_ammo_need_info_t *pAmmoInfo)
{
   if ((pent->v.effects & EF_NODRAW) || pent->v.frame > 0)
   {
      // someone owns this weapon or it hasn't respawned yet
   ...
   return BotGetAmmoPickupNeed(pBot, itemflag, pAmmoInfo) !=
      BOT_AMMO_NEED_NONE;
}


static void BotFindItemCheckTripmine(bot_t &pBot, edict_t *pent)
{
   edict_t *pEdict = pBot.pEdict;
...
static qboolean BotFindItem_ProcessFuncEntity(bot_t &pBot, edict_t *pent,
   const char *item_name, Vector &entity_origin, const Vector &vecStart)
{
   edict_t *pEdict = pBot.pEdict;
   Vector vecEnd = entity_origin;

   int angle_to_entity = BotInFieldOfView( pBot, vecEnd - vecStart );

   // check if entity is outside field of view (+/- 45 degrees)
   if (angle_to_entity > 45)
      return FALSE;  // skip this item if bot can't "see" it
   ...
   {
      return BotFindItemCheckFuncLadder(pBot, pent, entity_origin, vecStart);
   }
   else
   {
      ...
      int charger_result = BotFindItemCheckCharger(pBot, pent, item_name, distance, angle_to_entity, TRUE);
      if (charger_result == 2)
      {
         return TRUE;
      }
      ...
      {
         charger_result = BotFindItemCheckCharger(pBot, pent, item_name, distance, angle_to_entity, FALSE);
         if (charger_result == 2)
         {
            return TRUE;
         }
         else if (charger_result == 0)
         {
            return BotFindItemCheckFuncButton(pBot, pent, item_name, tr, distance, angle_to_entity);
         }
      }
   }
...
static void BotFindItem_CheckSpecialEntity(bot_t &pBot, edict_t *pent,
   const char *item_name)
{
   // check if entity is the spot from RPG laser
   if (strcmp("laser_spot", item_name) == 0)
   {
   }
   ...
   {
      BotFindItemCheckTripmine(pBot, pent);
   }
...
static qboolean BotFindItem_CheckHealthArmorItems(bot_t &pBot, edict_t *pent,
   edict_t *pEdict, const char *item_name)
{
   // pick up longjump
   if (strcmp("item_longjump", item_name) == 0)
   {
...
static const float BOT_PRIORITY_WEAPON_PROXIMITY_RADIUS = 192.0f;
static const float BOT_AMMO_OPPORTUNISTIC_RADIUS = 96.0f;
static const float BOT_AMMO_LOW_RADIUS = 192.0f;
static const float BOT_AMMO_CRITICAL_RADIUS = 256.0f;
static const float BOT_AMMO_OPPORTUNISTIC_MAX_DETOUR = 64.0f;
static const float BOT_PICKUP_SWITCH_DISTANCE_ADVANTAGE = 24.0f;
static const float BOT_AMMO_PASS_BY_RADIUS = 40.0f;

#define BOT_CLOSE_WEAPON_NOT_STRONG       0
#define BOT_CLOSE_WEAPON_AVAILABLE        1
#define BOT_CLOSE_WEAPON_INACTIVE         2
#define BOT_CLOSE_WEAPON_ALREADY_OWNED     3
#define BOT_CLOSE_WEAPON_AMMO_NOT_NEEDED   4
#define BOT_CLOSE_WEAPON_CANNOT_USE        5


static int BotFindItemCloseWeaponStatus(bot_t &pBot, edict_t *pItem,
   const char *item_name, int itemflag)
{
   if (!pItem || (pItem->v.effects & EF_NODRAW) || pItem->v.frame > 0)
      return BOT_CLOSE_WEAPON_INACTIVE;

   if (strcmp("weaponbox", item_name) == 0)
      return BOT_CLOSE_WEAPON_AVAILABLE;

   if (!itemflag)
      return BOT_CLOSE_WEAPON_NOT_STRONG;

   qboolean found_strong = FALSE;
   qboolean found_usable = FALSE;
   for (int i = 0; weapon_select[i].iId; i++)
   {
      bot_weapon_select_t &select = weapon_select[i];
      if (!(itemflag & select.waypoint_flag) || select.avoid_this_gun ||
          (select.type & WEAPON_FIRE) != WEAPON_FIRE ||
          BotIsWeakWeapon(select.iId))
         continue;

      found_strong = TRUE;
      if (!IsValidWeaponChoose(pBot, select) || !BotCanUseWeapon(pBot, select))
         continue;

      found_usable = TRUE;
      if (!BotIsCarryingWeapon(pBot, select.iId))
         return BOT_CLOSE_WEAPON_AVAILABLE;
      return BOT_CLOSE_WEAPON_ALREADY_OWNED;
   }

   if (found_strong && !found_usable)
      return BOT_CLOSE_WEAPON_CANNOT_USE;

   return BOT_CLOSE_WEAPON_NOT_STRONG;
}


static const char *BotFindItemCloseWeaponReason(int status)
{
   switch (status)
   {
      case BOT_CLOSE_WEAPON_INACTIVE: return "inactive";
      case BOT_CLOSE_WEAPON_ALREADY_OWNED: return "already_owned";
      case BOT_CLOSE_WEAPON_AMMO_NOT_NEEDED: return "ammo_not_needed";
      case BOT_CLOSE_WEAPON_CANNOT_USE: return "cannot_use";
      default: return "unknown";
   }
}


static void BotTraceCloseWeaponDecision(bot_t &pBot, const char *item_name,
   float distance, int angle, const char *reason, qboolean selected)
{
   if (bot_trace_level <= 0 ||
       (!selected && pBot.f_close_weapon_trace_time > gpGlobals->time))
      return;

   if (selected)
   {
      BotTrace(pBot,
         "close weapon intercepted: %s d=%.0f angle=%d old=%s/%d",
         item_name, distance, angle,
         BotTraceGoalTypeName(pBot.wpt_goal_type), pBot.waypoint_goal);
   }
   else
   {
      BotTrace(pBot, "close weapon rejected: %s d=%.0f angle=%d reason=%s",
         item_name, distance, angle, reason);
   }

   pBot.f_close_weapon_trace_time = gpGlobals->time + 1.0f;
}


static qboolean BotFindItemClosePickupReachable(bot_t &pBot,
   edict_t *pItem, const Vector &item_origin)
{
   TraceResult tr;
   UTIL_TraceHull(pBot.pEdict->v.origin, item_origin, ignore_monsters,
      human_hull, pBot.pEdict->v.pContainingEntity, &tr);
   return tr.flFraction > 0.999999f || tr.pHit == pItem;
}


static float BotAmmoNeedRadius(bot_ammo_need_t need)
{
   switch (need)
   {
      case BOT_AMMO_NEED_CRITICAL: return BOT_AMMO_CRITICAL_RADIUS;
      case BOT_AMMO_NEED_LOW: return BOT_AMMO_LOW_RADIUS;
      case BOT_AMMO_NEED_OPPORTUNISTIC:
         return BOT_AMMO_OPPORTUNISTIC_RADIUS;
      default: return 0.0f;
   }
}


static qboolean BotFindItemAmmoDetourAcceptable(const bot_t &pBot,
   const Vector &item_origin, bot_ammo_need_t need)
{
   if (need != BOT_AMMO_NEED_OPPORTUNISTIC ||
       pBot.curr_waypoint_index < 0 ||
       pBot.curr_waypoint_index >= num_waypoints)
      return TRUE;

   const Vector bot_origin = pBot.pEdict->v.origin;
   const Vector movement_target = waypoints[pBot.curr_waypoint_index].origin;
   const float direct_route = (movement_target - bot_origin).Length();
   const float item_route = (item_origin - bot_origin).Length() +
      (movement_target - item_origin).Length();
   return item_route - direct_route <= BOT_AMMO_OPPORTUNISTIC_MAX_DETOUR;
}


#define BOT_AMMO_TRACE_CANDIDATE    1
#define BOT_AMMO_TRACE_INTERCEPTED  2
...
static void BotTraceAmmoDecision(bot_t &pBot, edict_t *pItem,
   const bot_ammo_need_info_t &info, float distance, int angle, int event,
   const char *reason)
{
   if (bot_trace_level <= 0 || !pItem || pItem->v.classname == 0)
      return;
   ...
   if (reason)
   {
      for (const char *p = reason; *p; p++)
         reason_code = reason_code * 33 + *p;
   }

   if (pBot.pAmmoTraceItem == pItem &&
   ...
   {
      BotTrace(pBot,
         "ammo candidate: %s need=%s idx=%d current=%d max=%d d=%.0f angle=%d goal=%s movement=%s",
         item_name, BotAmmoNeedName(info.need), info.ammo_index,
         info.current, info.maximum, distance, angle,
         BotTraceGoalTypeName(pBot.wpt_goal_type),
         BotMovementModeName(pBot.movement_mode));
   }
   else if (event == BOT_AMMO_TRACE_INTERCEPTED)
   {
      BotTrace(pBot, "ammo intercepted: %s need=%s d=%.0f angle=%d old=%s/%d",
         item_name, BotAmmoNeedName(info.need), distance, angle,
         BotTraceGoalTypeName(pBot.wpt_goal_type), pBot.waypoint_goal);
   }
   else if (event == BOT_AMMO_TRACE_PASS_BY)
   {
      BotTrace(pBot,
         "ammo_pass_by: %s need=%s idx=%d current=%d max=%d d=%.0f angle=%d",
         item_name, BotAmmoNeedName(info.need), info.ammo_index,
         info.current, info.maximum, distance, angle);
   }
   else
   {
      BotTrace(pBot,
         "ammo skipped: %s reason=%s current=%d max=%d d=%.0f",
         item_name, reason ? reason : "unknown", info.current,
         info.maximum, distance);
   }

   pBot.pAmmoTraceItem = pItem;
   pBot.trace_last_ammo_event = event;
   pBot.trace_last_ammo_reason = reason_code;
   pBot.trace_last_ammo_need = info.need;
   pBot.f_ammo_trace_time = gpGlobals->time +
...
static qboolean BotHasPriorityWeaponPickup(bot_t &pBot)
{
   edict_t *pItem = pBot.pBotPickupItem;
   if (!pItem || !BotFindItemIsWeaponPickup(pItem) ||
       (!pBot.b_only_has_weak_weapons &&
        !BotIsInSpawnWeaponSearchWindow(pBot)))
      return FALSE;

   const char *item_name = STRING(pItem->v.classname);
   int itemflag = GetWeaponItemFlag(item_name);
   return BotFindItemCloseWeaponStatus(pBot, pItem, item_name, itemflag) ==
      BOT_CLOSE_WEAPON_AVAILABLE;
}


static qboolean BotHasPriorityAmmoPickup(bot_t &pBot)
{
   edict_t *pItem = pBot.pBotPickupItem;
   if (!pItem || pItem->free || (pItem->v.effects & EF_NODRAW) ||
       pItem->v.frame > 0)
      return FALSE;

   bot_ammo_need_info_t info;
   bot_ammo_need_t need = BotFindItemGetAmmoSourceNeed(pBot, pItem, &info);
   if (need == BOT_AMMO_NEED_NONE)
      return FALSE;

   // Radius gates initial opportunistic interception. Once low/critical ammo
   // has been selected, combat aim must not cancel locomotion to it.
   return need != BOT_AMMO_NEED_OPPORTUNISTIC ||
      (pItem->v.origin - pBot.pEdict->v.origin).Length() <=
         BotAmmoNeedRadius(need);
}


static qboolean BotHasPriorityCombatPickup(bot_t &pBot)
{
   return BotHasPriorityWeaponPickup(pBot) ||
      BotHasPriorityAmmoPickup(pBot);
}


static int BotFindItem_GetPriority(const bot_t &pBot, const char *item_name,
   bot_ammo_need_t ammo_need, qboolean close_weapon)
{
   if (strcmp("item_healthkit", item_name) == 0 &&
       pBot.pEdict->v.health <= 25.0f)
      return 80;

   if (close_weapon)
      return 70;

   // A weak loadout keeps weapon upgrades ahead of every ammo tier.
   if ((BotIsInSpawnWeaponSearchWindow(pBot) ||
        pBot.b_only_has_weak_weapons) &&
       (GetWeaponItemFlag(item_name) != 0 ||
        strcmp("weaponbox", item_name) == 0) &&
       ammo_need == BOT_AMMO_NEED_NONE)
      return 60;

   if (ammo_need == BOT_AMMO_NEED_CRITICAL)
      return 50;
   if (ammo_need == BOT_AMMO_NEED_LOW)
      return 40;
   if (ammo_need == BOT_AMMO_NEED_OPPORTUNISTIC)
      return 30;
...
static void BotFindItem( bot_t &pBot )
{
   ...
   edict_t *pOriginalPickup = pBot.pBotPickupItem;
   edict_t *pBestAmmoEntity = NULL;
   Vector pickup_origin;
   Vector entity_origin;
   float radius = 500.0f;
   float min_distance;
   float best_ammo_distance = 99999.0f;
   float original_distance = 99999.0f;
   int best_priority = 0;
   int best_ammo_priority = 0;
   int original_priority = 0;
   int pickup_angle = 0;
   int best_ammo_angle = 0;
   qboolean pickup_is_close_weapon = FALSE;
   bot_ammo_need_info_t pickup_ammo_info;
   bot_ammo_need_info_t best_ammo_info;
   char item_name[40];
   edict_t *pEdict = pBot.pEdict;

   memset(&pickup_ammo_info, 0, sizeof(pickup_ammo_info));
   memset(&best_ammo_info, 0, sizeof(best_ammo_info));

   if (BotFindItemCheckExpiry(pBot))
      return;

   pOriginalPickup = pBot.pBotPickupItem;
   ...
   while ((pent = UTIL_FindEntityInSphere(pent, pEdict->v.origin,
      radius)) != NULL)
   {
      qboolean can_pickup = FALSE;
      qboolean close_weapon = FALSE;
      qboolean ammo_source = FALSE;
      bot_ammo_need_info_t ammo_info;
      memset(&ammo_info, 0, sizeof(ammo_info));
      ammo_info.ammo_index = -1;
      ammo_info.weapon_id = -1;
      ammo_info.reason = BOT_AMMO_REASON_IRRELEVANT;
      ...
      if (strncmp("func_", item_name, 5) == 0)
      {
         ...
         Vector vec_start = pEdict->v.origin + pEdict->v.view_ofs;
         can_pickup = BotFindItem_ProcessFuncEntity(pBot, pent, item_name,
            entity_origin, vec_start);
      }
      else
      {
         entity_origin = pent->v.origin;
         Vector vec_start = pEdict->v.origin + pEdict->v.view_ofs;
         int angle_to_entity = BotInFieldOfView(pBot,
            entity_origin - vec_start);
         float distance = (entity_origin - pEdict->v.origin).Length();
         int weapon_item_flags = GetWeaponItemFlag(item_name);
         int ammo_item_flags = GetAmmoItemFlag(item_name);
         bot_ammo_need_t ammo_need = BOT_AMMO_NEED_NONE;

         if (ammo_item_flags)
         {
            ammo_source = TRUE;
            BotFindItemCheckAmmo(pBot, pent, ammo_item_flags, &ammo_info);
            ammo_need = ammo_info.need;
         }
         else if (weapon_item_flags)
         {
            ammo_need = BotGetWeaponRepickupAmmoNeed(pBot,
               weapon_item_flags, &ammo_info);
            ammo_source = ammo_info.ammo_item_flags != 0;
         }

         const qboolean priority_search = pBot.b_only_has_weak_weapons ||
            BotIsInSpawnWeaponSearchWindow(pBot);

         if (priority_search &&
             distance <= BOT_PRIORITY_WEAPON_PROXIMITY_RADIUS &&
             (weapon_item_flags != 0 || strcmp("weaponbox", item_name) == 0))
         {
            int close_status = BotFindItemCloseWeaponStatus(pBot, pent,
               item_name, weapon_item_flags);
            if (close_status == BOT_CLOSE_WEAPON_AVAILABLE)
            {
               close_weapon = TRUE;
               if (!BotEntityIsVisible(pBot, entity_origin) ||
                   !BotFindItemClosePickupReachable(pBot, pent,
                      entity_origin))
               {
                  BotTraceCloseWeaponDecision(pBot, item_name, distance,
                     angle_to_entity, "blocked_los", FALSE);
                  continue;
               }
            }
            else if (!ammo_source &&
                     close_status != BOT_CLOSE_WEAPON_NOT_STRONG)
            {
               BotTraceCloseWeaponDecision(pBot, item_name, distance,
                  angle_to_entity,
                  BotFindItemCloseWeaponReason(close_status), FALSE);
               continue;
            }
         }
         ...
         if (ammo_source)
         {
            ...
            if (ammo_need == BOT_AMMO_NEED_NONE)
            {
               if (distance <= BOT_AMMO_CRITICAL_RADIUS)
                  BotTraceAmmoDecision(pBot, pent, ammo_info, distance,
                     angle_to_entity, BOT_AMMO_TRACE_SKIPPED,
                     BotAmmoNeedReasonName(ammo_info.reason));
               continue;
            }
            ...
            const float proximity_radius = BotAmmoNeedRadius(ammo_need);
            const qboolean proximity_override = distance <= proximity_radius;

            // Opportunistic ammo is strictly local. Low/critical ammo may
            // still use the ordinary FOV scan beyond their intercept radius.
            if (!proximity_override &&
                ammo_need == BOT_AMMO_NEED_OPPORTUNISTIC)
            {
               if (distance <= BOT_AMMO_CRITICAL_RADIUS)
                  BotTraceAmmoDecision(pBot, pent, ammo_info, distance,
                     angle_to_entity, BOT_AMMO_TRACE_SKIPPED,
            ...
            // Remember an already-vetted low/critical target while looking at
            // an enemy; LOS and hull reachability are still rechecked below.
            if (!proximity_override && angle_to_entity > 45 &&
                pent != pOriginalPickup)
               continue;

            if (!BotEntityIsVisible(pBot, entity_origin))
            {
               BotTraceAmmoDecision(pBot, pent, ammo_info, distance,
                  angle_to_entity, BOT_AMMO_TRACE_SKIPPED, "blocked_los");
               continue;
            }

            if (!BotFindItemClosePickupReachable(pBot, pent, entity_origin))
            {
               BotTraceAmmoDecision(pBot, pent, ammo_info, distance,
                  angle_to_entity, BOT_AMMO_TRACE_SKIPPED, "unreachable");
               continue;
            }

            if (!BotFindItemAmmoDetourAcceptable(pBot, entity_origin,
                ammo_need))
            {
               BotTraceAmmoDecision(pBot, pent, ammo_info, distance,
                  angle_to_entity, BOT_AMMO_TRACE_SKIPPED,
            ...
            can_pickup = TRUE;
            BotTraceAmmoDecision(pBot, pent, ammo_info, distance,
               angle_to_entity, BOT_AMMO_TRACE_CANDIDATE, "useful");
         }
         else
         {
            if (!close_weapon && angle_to_entity > 45)
               continue;
            if (!close_weapon && !BotEntityIsVisible(pBot, entity_origin))
               continue;

            if (weapon_item_flags != 0)
               can_pickup = BotFindItemCheckWeapon(pBot, pent,
                  weapon_item_flags, &ammo_info);
            else
            {
               can_pickup = BotFindItem_CheckHealthArmorItems(pBot, pent,
                  pEdict, item_name);
               if (!can_pickup)
                  BotFindItem_CheckSpecialEntity(pBot, pent, item_name);
            }
         }
      }
      ...
      float distance = (entity_origin - pEdict->v.origin).Length();
      int angle = BotInFieldOfView(pBot, entity_origin -
         (pEdict->v.origin + pEdict->v.view_ofs));

      if (BotFindItemClaimedByVisibleBot(pBot, pent))
      {
         if (ammo_source)
            BotTraceAmmoDecision(pBot, pent, ammo_info, distance, angle,
               BOT_AMMO_TRACE_SKIPPED, "claimed");
         else if (distance <= BOT_PRIORITY_WEAPON_PROXIMITY_RADIUS &&
                  (GetWeaponItemFlag(item_name) != 0 ||
                   strcmp("weaponbox", item_name) == 0))
            BotTraceCloseWeaponDecision(pBot, item_name, distance, angle,
               "claimed", FALSE);
         continue;
      }

      int priority = BotFindItem_GetPriority(pBot, item_name,
         ammo_info.need, close_weapon);

      if (ammo_source &&
          (priority > best_ammo_priority ||
           (priority == best_ammo_priority &&
            distance < best_ammo_distance)))
      {
         pBestAmmoEntity = pent;
         best_ammo_info = ammo_info;
         best_ammo_distance = distance;
         best_ammo_angle = angle;
         best_ammo_priority = priority;
      }

      if (pent == pOriginalPickup)
      {
          ...
          (priority == best_priority && distance < min_distance))
      {
         ...
         pickup_angle = angle;
         pickup_is_close_weapon = close_weapon;
         pickup_ammo_info = ammo_info;
      }
   }

   // A same-tier replacement needs a meaningful distance advantage. This
   // keeps two nearby pickups from causing 10 Hz target oscillation.
   if (pOriginalPickup && pPickupEntity &&
       pOriginalPickup != pPickupEntity && original_priority > 0 &&
       (original_priority > best_priority ||
        (original_priority == best_priority &&
         min_distance + BOT_PICKUP_SWITCH_DISTANCE_ADVANTAGE >=
            original_distance)))
   {
      pPickupEntity = pOriginalPickup;
      pickup_origin = pOriginalPickup->v.origin;
      min_distance = original_distance;
      best_priority = original_priority;
      pickup_angle = BotInFieldOfView(pBot, pickup_origin -
         (pEdict->v.origin + pEdict->v.view_ofs));
      pickup_is_close_weapon = BotHasPriorityWeaponPickup(pBot);
      BotFindItemGetAmmoSourceNeed(pBot, pOriginalPickup,
         &pickup_ammo_info);
   }

   if (pBestAmmoEntity && pPickupEntity != pBestAmmoEntity &&
       best_priority >= best_ammo_priority)
   {
      BotTraceAmmoDecision(pBot, pBestAmmoEntity, best_ammo_info,
         best_ammo_distance, best_ammo_angle, BOT_AMMO_TRACE_SKIPPED,
         "higher_priority_pickup");
   }

   if (pBestAmmoEntity && pPickupEntity == NULL &&
       best_ammo_distance <= BOT_AMMO_PASS_BY_RADIUS)
   {
      BotTraceAmmoDecision(pBot, pBestAmmoEntity, best_ammo_info,
         best_ammo_distance, best_ammo_angle, BOT_AMMO_TRACE_PASS_BY,
         "pass_by");
   }

   if (!pPickupEntity && pOriginalPickup &&
       BotFindItemGetAmmoSourceNeed(pBot, pOriginalPickup, NULL) !=
          BOT_AMMO_NEED_NONE)
   {
      pBot.pBotPickupItem = NULL;
   ...
   {
      if (pickup_is_close_weapon && pBot.pBotPickupItem != pPickupEntity)
         BotTraceCloseWeaponDecision(pBot,
            STRING(pPickupEntity->v.classname), min_distance,
            pickup_angle, "selected", TRUE);

      if (pickup_ammo_info.need != BOT_AMMO_NEED_NONE &&
          pBot.pBotPickupItem != pPickupEntity)
         BotTraceAmmoDecision(pBot, pPickupEntity, pickup_ammo_info,
            min_distance, pickup_angle, BOT_AMMO_TRACE_INTERCEPTED,
            "selected");

      BotFindItem_SetPickupTarget(pBot, pPickupEntity, pickup_origin);
   }
}
...
//
static qboolean BotCheckLogoSpraying_TraceWallDirection(edict_t *pEdict, const Vector &v_src, const Vector &v_direction, float distance)
{
   TraceResult tr;
   Vector v_dest = v_src + v_direction * distance;
...
static void BotCheckLogoSpraying_ExecuteSpray(bot_t &pBot)
{
   edict_t *pEdict = pBot.pEdict;
   TraceResult tr;
   ...
   Vector v_dest = v_src + UTIL_AnglesToForward(pEdict->v.v_angle) * 80;
...
static void BotCheckLogoSpraying(bot_t &pBot)
{
   ...
   if (pBot.b_spray_logo)  // trying to spray a logo?
   {
      ...
      else if (!BotCheckLogoSpraying_TraceWallDirection(pEdict, v_src, gpGlobals->v_right, 100))  // to the right
      {
         if (!BotCheckLogoSpraying_TraceWallDirection(pEdict, v_src, gpGlobals->v_right * -1, 100))  // to the left
         {
            if (!BotCheckLogoSpraying_TraceWallDirection(pEdict, v_src, gpGlobals->v_forward * -1, 100))  // behind
            {
               // on the ground...
      ...
      BotCheckLogoSpraying_ExecuteSpray(pBot);
   }
...
static void BotWanderHandleStation(bot_t &pBot, qboolean &b_use_station, float f_use_time)
{
   edict_t *pEdict = pBot.pEdict;

   if ((f_use_time + 10.0) > gpGlobals->time)
   {
      pBot.f_move_speed = 0;  // don't move while using station

      // block strafing before using station
   ...
   {
      // bot is stuck trying to "use" a station...
...
static void BotWanderCheckWalls(bot_t &pBot, float &moved_distance)
{
   edict_t *pEdict = pBot.pEdict;
   TraceResult tr;
...
static void BotWanderHandleStuck(bot_t &pBot, float moved_distance)
{
   edict_t *pEdict = pBot.pEdict;
...
static void BotWanderFreeRoam(bot_t &pBot, float moved_distance)
{
   ...
   {
      BotWanderCheckWalls(pBot, moved_distance);
   }
   ...
   BotWanderHandleStuck(pBot, moved_distance);
...
//
static qboolean BotJustWander_HandleTripmines(bot_t &pBot)
{
   edict_t *pEdict = pBot.pEdict;
...
static void BotJustWander_HandleStationsAndLifts(bot_t &pBot, float moved_distance)
{
   // check if should use wall mounted health station...
   if (pBot.b_use_health_station)
   {
      BotWanderHandleStation(pBot, pBot.b_use_health_station, pBot.f_use_health_time);
   }

   // check if should use wall mounted HEV station...
   else if (pBot.b_use_HEV_station)
   {
      BotWanderHandleStation(pBot, pBot.b_use_HEV_station, pBot.f_use_HEV_time);
   }
   ...
   {
      BotWanderFreeRoam(pBot, moved_distance);
   }
}
...
static void BotJustWanderAround(bot_t &pBot, float moved_distance)
{
   ...
   if (!BotJustWander_HandleTripmines(pBot))
   {
      BotJustWander_HandleStationsAndLifts(pBot, moved_distance);
   }
}
...
static void BotDoStrafeCombat_SelectDirection(bot_t &pBot)
{
   ...
   if(isMelee)
   {
      ...
      {
         pBot.f_strafe_time = gpGlobals->time + RANDOM_FLOAT2(0.3, 0.7);
         pBot.f_strafe_direction = RANDOM_FLOAT(-0.2, 0.2);
      }
   ...
   // strafe instead
   if(RANDOM_LONG2(1, 100) <= BotSkillGetBattleStrafe(pBot.bot_skill))
   {
      if(pBot.f_strafe_time <= gpGlobals->time)
      {
...
static void BotDoStrafeCombat_WallCollisionCheck(bot_t &pBot)
{
   edict_t *pEdict = pBot.pEdict;
...
static void BotDoStrafeCombat(bot_t &pBot)
{
   BotDoStrafeCombat_SelectDirection(pBot);

   BotDoStrafeCombat_WallCollisionCheck(pBot);
...
static void BotDoStrafeLadder_AlignWithVelocity(bot_t &pBot)
{
   edict_t *pEdict = pBot.pEdict;
...
static void BotDoStrafeLadder(bot_t &pBot)
{
   edict_t *pEdict = pBot.pEdict;
   ...
   if(pBot.curr_waypoint_index != -1 && waypoints[pBot.curr_waypoint_index].flags & W_FL_LADDER)
   {
      ...
      {
         BotDoStrafeLadder_AlignWithVelocity(pBot);
      }
      else
      {
...
static const char *BotMovementModeName(int movement_mode)
{
   ...
   {
      case BOT_MOVE_WEAK_PHYSICAL_PICKUP: return "weak_physical_pickup";
      case BOT_MOVE_WEAK_WEAPON_ROUTE: return "weak_weapon_route";
      case BOT_MOVE_WEAK_RETREAT: return "weak_retreat";
      case BOT_MOVE_WEAK_ROAM: return "weak_roam";
      case BOT_MOVE_NORMAL_COMBAT: return "normal_combat";
      case BOT_MOVE_CROSSFIRE_STRATEGIC: return "crossfire_strategic";
      case BOT_MOVE_PRIORITY_AMMO_PICKUP: return "priority_ammo_pickup";
      case BOT_MOVE_AMMO_ROUTE: return "ammo_route";
      default: return "unset";
   }
}


static int BotSelectMovementMode(bot_t &pBot)
{
   if (MapProfileShouldYieldToStrategicMovement(pBot))
      return BOT_MOVE_CROSSFIRE_STRATEGIC;

   if (pBot.b_only_has_weak_weapons && BotHasPriorityWeaponPickup(pBot))
      return BOT_MOVE_WEAK_PHYSICAL_PICKUP;

   if (BotHasPriorityAmmoPickup(pBot))
      return BOT_MOVE_PRIORITY_AMMO_PICKUP;

   if (pBot.wpt_goal_type == WPT_GOAL_AMMO)
      return BOT_MOVE_AMMO_ROUTE;

   if (pBot.b_only_has_weak_weapons)
   {
      if (pBot.wpt_goal_type == WPT_GOAL_WEAPON)
         return BOT_MOVE_WEAK_WEAPON_ROUTE;
      if (pBot.b_weak_retreat_goal)
         return BOT_MOVE_WEAK_RETREAT;
      return BOT_MOVE_WEAK_ROAM;
   }

   return BOT_MOVE_NORMAL_COMBAT;
}


static void BotTraceMovementMode(bot_t &pBot)
{
   const qboolean ammo_movement =
      pBot.movement_mode == BOT_MOVE_PRIORITY_AMMO_PICKUP ||
      pBot.movement_mode == BOT_MOVE_AMMO_ROUTE;
   if ((!pBot.b_only_has_weak_weapons && !ammo_movement) ||
       bot_trace_level <= 0)
      return;

   if (pBot.trace_last_movement_mode == pBot.movement_mode &&
       pBot.f_movement_trace_time > gpGlobals->time)
      return;

   const char *pickup_classname = "none";
   float pickup_distance = -1.0f;
   if (pBot.pBotPickupItem && !pBot.pBotPickupItem->free &&
       pBot.pBotPickupItem->v.classname != 0)
   {
      pickup_classname = STRING(pBot.pBotPickupItem->v.classname);
      pickup_distance = (pBot.pBotPickupItem->v.origin -
         pBot.pEdict->v.origin).Length();
   }

   float enemy_distance = -1.0f;
   if (!FNullEnt(pBot.pBotEnemy))
      enemy_distance = (UTIL_GetOriginWithExtent(pBot, pBot.pBotEnemy) -
         pBot.pEdict->v.origin).Length();

   const int zone_load = ammo_movement ? pBot.ammo_goal_zone_load :
      pBot.weapon_goal_zone_load;
   BotTrace(pBot,
      "movement mode=%s goal=%s wpt=%d pickup=%s pd=%.0f enemy=%.0f load=%d",
      BotMovementModeName(pBot.movement_mode),
      BotTraceGoalTypeName(pBot.wpt_goal_type), pBot.waypoint_goal,
      pickup_classname, pickup_distance, enemy_distance,
      zone_load);

   pBot.trace_last_movement_mode = pBot.movement_mode;
   pBot.f_movement_trace_time = gpGlobals->time + 2.0f;
}


static void BotSuppressWeakEnemyMovementGoal(bot_t &pBot)
{
   if (!pBot.b_only_has_weak_weapons ||
       pBot.wpt_goal_type != WPT_GOAL_ENEMY)
      return;

   BotTrace(pBot, "weak enemy goal suppressed before locomotion: wpt=%d",
      pBot.waypoint_goal);
   pBot.waypoint_goal = -1;
   pBot.wpt_goal_type = WPT_GOAL_NONE;
   pBot.curr_waypoint_index = -1;
...
static qboolean BotDoStrafeGoalDirectedCombat(bot_t &pBot)
{
   const qboolean strategic_movement =
      MapProfileShouldYieldToStrategicMovement(pBot);
   const qboolean weak_combat_movement = pBot.pBotEnemy != NULL &&
      pBot.b_only_has_weak_weapons;
   const qboolean ammo_combat_movement = pBot.pBotEnemy != NULL &&
      (BotHasPriorityAmmoPickup(pBot) ||
       pBot.wpt_goal_type == WPT_GOAL_AMMO);

   pBot.movement_mode = BotSelectMovementMode(pBot);
   BotTraceMovementMode(pBot);

   if (!strategic_movement && !weak_combat_movement &&
       !ammo_combat_movement)
      return FALSE;
   ...
   if (has_movement_target && pBot.f_move_speed != 0.0f)
   {
   ...
   else if (!has_movement_target && weak_combat_movement)
   {
      // With no valid route, back away or strafe while still aiming and
      // returning fire. Never let ordinary combat close the distance.
      if (pBot.f_strafe_time <= gpGlobals->time)
      {
         pBot.f_strafe_time = gpGlobals->time +
            RANDOM_FLOAT2(0.4f, 0.8f);
         pBot.f_strafe_direction =
            RANDOM_LONG2(0, 1) == 0 ? -0.75f : 0.75f;
      }

      float retreat_speed = fabs(pBot.f_move_speed);
      if (retreat_speed < 1.0f)
         retreat_speed = pBot.f_max_speed;

      pBot.f_move_direction = -1.0f;
      pBot.f_move_speed = BotCheckWallOnBack(pBot) ? 0.0f :
         -retreat_speed;
   }
...
static void BotDoStrafe(bot_t &pBot)
{
   ...
   BotSuppressWeakEnemyMovementGoal(pBot);
...
static qboolean BotRandomMovementCanStart(bot_t &pBot)
{
   edict_t *pEdict = pBot.pEdict;
   ...
   // Don't random-jump away from a committed tactical pickup.
   if(!FNullEnt(pBot.pBotPickupItem) &&
      !(pBot.pBotPickupItem->v.effects & EF_NODRAW) &&
      !(pBot.pBotPickupItem->v.frame > 0) &&
      (pBot.pBotEnemy == NULL || BotHasPriorityCombatPickup(pBot)))
      return FALSE;
...
static qboolean BotRandomMovementStartLongjump(bot_t &pBot, float max_lj_distance)
{
   edict_t *pEdict = pBot.pEdict;
...
static void BotDoRandomMovement_EvaluateConditions(bot_t &pBot, float moved_distance, qboolean &jump, qboolean &duck, qboolean &lj, float &max_lj_distance)
{
   edict_t *pEdict = pBot.pEdict;
...
static qboolean BotDoRandomMovement_SelectAction(bot_t &pBot, qboolean &jump, qboolean &duck, qboolean &lj)
{
...
static void BotDoRandomMovement_ExecuteAction(bot_t &pBot, qboolean jump, qboolean duck, qboolean lj, float max_lj_distance)
{
   edict_t *pEdict = pBot.pEdict;
   ...
   if(lj)
   {
      BotRandomMovementStartLongjump(pBot, max_lj_distance);
   }
}
...
static void BotDoRandomJumpingAndDuckingAndLongJumping(bot_t &pBot, float moved_distance)
{
   ...
   //
   if (BotRandomMovementHandlePending(pBot))
      return;

   //
   // PART2 -- return here if actions unfinished
   //
   if (!BotRandomMovementCanStart(pBot))
      return;
   ...
   float max_lj_distance = 0.0;

   BotDoRandomMovement_EvaluateConditions(pBot, moved_distance, jump, duck, lj, max_lj_distance);

   if (BotDoRandomMovement_SelectAction(pBot, jump, duck, lj))
      return;

   BotDoRandomMovement_ExecuteAction(pBot, jump, duck, lj, max_lj_distance);
}
...
static void BotThinkUpdatePhysicsState(bot_t &pBot)
{
   edict_t *pEdict = pBot.pEdict;
...
static void BotThinkHandleEnemy_RunawayLogic(bot_t &pBot, edict_t *pAvoidEnemy)
{
   edict_t *pEdict = pBot.pEdict;
...
static qboolean BotThinkHandleEnemy(bot_t &pBot)
{
   edict_t *pEdict = pBot.pEdict;
   ...
   if (pBot.pBotEnemy != NULL)
   {
      ...
      else
      {
         ...
         BotThinkHandleEnemy_RunawayLogic(pBot, pBot.pBotEnemy);
      }
   }

   return DidShootAtEnemy;
}
...
static void BotThinkHandleNavigation(bot_t &pBot, qboolean did_shoot, float moved_distance)
{
   ...
   const qboolean weapon_acquisition_movement = did_shoot &&
      (pBot.b_only_has_weak_weapons || BotHasPriorityCombatPickup(pBot) ||
       pBot.wpt_goal_type == WPT_GOAL_AMMO);
...
static void BotThinkAdjustSpeedForWaypoint(bot_t &pBot)
{
   edict_t *pEdict = pBot.pEdict;
...
static void BotThinkFinalizeMovement(bot_t &pBot)
{
   edict_t *pEdict = pBot.pEdict;
...
void BotThink( bot_t &pBot )
{
   ...
   BotThinkUpdateFrameTiming(pBot);

   BotThinkUpdatePhysicsState(pBot);
   ...
   qboolean DidShootAtEnemy = BotThinkHandleEnemy(pBot);

   BotThinkHandleNavigation(pBot, DidShootAtEnemy, moved_distance);

   BotThinkAdjustSpeedForWaypoint(pBot);
